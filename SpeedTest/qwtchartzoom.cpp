    ...
    : QObject(qp)
{
    ...
    // получаем компонент QwtPlot, над которым будут производиться все
    // преобразования
    qwtp = qp;
    // устанавливаем ему свойство, разрешающее обрабатывать события от
    // клавиатуры
    ...
    if (grid != NULL)
    {
        ...
        // горизонтальную
        allocAxis(
            xAx, QwtPlot::xBottom + QwtPlot::xTop - xAx, &masterX, &slaveX);
        // вертикальную
        allocAxis(
            yAx, QwtPlot::yLeft + QwtPlot::yRight - yAx, &masterY, &slaveY);
    }
...
QObject*
QwtChartZoom::generalParent(QObject* p)
{
...
void
QwtChartZoom::allocAxis(int pre, int alt, QwtPlot::Axis* master,
                        QwtPlot::Axis* slave)
{
    ...
    if ((qwtp->axisEnabled(pre) || !qwtp->axisEnabled(alt))
        && (smp.s1() != smp.s2() || sma.s1() == sma.s2()))
    {
...
void
QwtChartZoom::setRegim(QwtChartZoom::QConvType ct)
{
    convType = ct;
}
...
void
QwtChartZoom::setRubberBandColor(QColor clr)
{
...
void
QwtChartZoom::setLightMode(bool lm)
{
...
void
QwtChartZoom::indicateDragBand(QDragIndiStyle indi)
{
...
void
QwtChartZoom::setDragBandColor(QColor clr)
{
...
void
QwtChartZoom::setHorSync(bool hs)
{
...
void
QwtChartZoom::setVerSync(bool vs)
{
    // если состояние синхронизации изменилось, то
    // перестраиваем график
    if (setVSync(vs))
...
QList<int>*
QwtChartZoom::getLabelWidths(int xAx)
{
    ...
    if (res->count() > 2) // если в списке больше 2-х меток
    {
        ...
        for (int k = 1; k < res->count(); k++)
        {
            ...
            // проверяем значение
            if (wk < mn)
                mn = wk; // минимальной ширины
            if (wk > mx)
                mx = wk; // и максимальной
        }
...
int
QwtChartZoom::meanLabelWidth(int xAx)
{
    ...
    for (int k = 0; k < lbwds->count(); k++)
        res += lbwds->at(k);
    // и определяем среднюю ширину
    if (lbwds->count() > 0)
        res = floor(res / lbwds->count());
...
bool
QwtChartZoom::updateHorAxis(int xAx, int* hDiv)
{
...
bool
QwtChartZoom::updateVerAxis(int yAx, int* vDiv)
{
    ...
    // определяем размер надписи, соответствующей этому значению при заданном
    // шрифте
#if QWT_VERSION < 0x060000
...
bool
QwtChartZoom::eventFilter(QObject* target, QEvent* event)
{
...
// Конструктор
QwtChartZoom::QScaleBounds::QScaleBounds(QwtPlot* plt, QwtPlot::Axis mst,
                                         QwtPlot::Axis slv)
{
    // запоминаем
...
void
QwtChartZoom::QScaleBounds::set(double mn, double mx)
{
...
void
QwtChartZoom::QScaleBounds::rest()
{
...
bool
QwtChartZoom::QScaleBounds::affected(QwtPlot::Axis ax)
{
...
void
QwtChartZoom::QScaleBounds::setDiv(QwtScaleDiv* sdv)
{
    ...
    if (sync)
        plot->setAxisScaleDiv(slave, *sdv);
}
...
void
QMainZoomSvc::attach(QwtChartZoom* zm)
{
...
// Установка цвета рамки, задающей новый размер графика
void
QMainZoomSvc::setRubberBandColor(QColor clr)
{
    zwClr = clr;
}
...
// Обработчик всех событий
bool
QMainZoomSvc::eventFilter(QObject* target, QEvent* event)
{
    // если событие произошло для графика, то
    if (target == zoom->plot())
...
void
QMainZoomSvc::showZoomWidget(QRect zr)
{
    // устанавливаем положение и размеры виджета, отображающего выделенную
    // область
    zwid->setGeometry(zr);
...
void
QMainZoomSvc::procMouseEvent(QEvent* event)
{
...
void
QMainZoomSvc::startZoom(QMouseEvent* mEvent)
{
    ...
    if (zoom->regim() == QwtChartZoom::ctNone)
    {
        ...
        QwtPlotCanvas* cv
            = qobject_cast<QwtPlotCanvas*>(plt->canvas()); // и канву
        ...
        if (scp_x >= 0 && scp_x < cg.width() && scp_y >= 0
            && scp_y < cg.height())
            ...
            if (mEvent->button() == Qt::LeftButton)
            {
                ...
                zwid->setStyleSheet(QString("background-color:rgb(%1,%2,%3);")
                                        .arg(zwClr.red())
                                        .arg(zwClr.green())
                                        .arg(zwClr.blue()));
            }
...
void
QMainZoomSvc::selectZoomRect(QMouseEvent* mEvent)
{
    ...
    if (zoom->regim() == QwtChartZoom::ctZoom)
    {
        ...
        //     относительно виджета, его содержащего
        // pg.x() + cg.x() + scp_x - положение gx0 начальной точки по
        // горизонтальной оси
        //     относительно виджета, содержащего график, она нужна в качестве
        //     опоры для отображения выделенной области
        int gx0 = pg.x() + cg.x() + scp_x;
        // если ширина выделенной области отрицательна, то текущая точка
        // находится левее начальной,
        //     и тогда именно ее мы используем в качестве опоры для отображения
        //     выделенной области
        if (dx < 0)
        {
            dx = -dx;
        ...
        // иначе если ширина равна нулю, то для того чтобы выделенная область
        // все-таки отбражалась,
        ...
        // если высота выделенной области отрицательна, то текущая точка
        // находится выше начальной,
        //     и тогда именно ее мы используем в качестве опоры для отображения
        //     выделенной области
        if (dy < 0)
        {
        ...
        // иначе если высота равна нулю, то для того чтобы выделенная область
        // все-таки отбражалась,
        //     принудительно сделаем ее равной единице
        else if (dy == 0)
            dy = 1;
...
void
QMainZoomSvc::procZoom(QMouseEvent* mEvent)
{
    ...
    if (zoom->regim() == QwtChartZoom::ctZoom)
        ...
        if (mEvent->button() == Qt::LeftButton)
        {
            ...
            // конечной точки выделенной области (в пикселах относительно канвы
            // QwtPlot)
            ...
            if (xp < scp_x || yp < scp_y)
                zoom->restBounds();
            // иначе если размер выделенной области достаточен, то изменяем
            // масштаб
            else if (xp - scp_x >= 8 && yp - scp_y >= 8)
            {
                QwtPlot::Axis mX = zoom->masterH();
                // определяем левую границу горизонтальной шкалы по начальной
                // точке
                double lf = plt->invTransform(mX, scp_x);
                // определяем правую границу горизонтальной шкалы по конечной
                // точке
                ...
                // определяем нижнюю границу вертикальной шкалы по конечной
                // точке
                double bt = plt->invTransform(mY, yp);
                // определяем верхнюю границу вертикальной шкалы по начальной
                // точке
...
// Прикрепление интерфейса к менеджеру масштабирования
void
QDragZoomSvc::attach(QwtChartZoom* zm)
{
    // запоминаем указатель на менеджер масштабирования
    zoom = zm;
    // назначаем для графика обработчик событий (фильтр событий)
...
// Включение/выключение легкого режима
void
QDragZoomSvc::setLightMode(bool lm)
{
    light = lm;
}
...
// Установка цвета виджета индикатора перемещения
void
QDragZoomSvc::setDragBandColor(QColor clr)
{
    dwClr = clr;
}
...
// (имеет эффект, если включен легкий режим)
void
QDragZoomSvc::setIndicatorStyle(QwtChartZoom::QDragIndiStyle indi)
{
    indiDrB = indi;
}
...
// Обработчик всех событий
bool
QDragZoomSvc::eventFilter(QObject* target, QEvent* event)
{
    // если событие произошло для графика, то
    if (target == zoom->plot())
        // если произошло одно из событий от мыши, то
...
// Добавление в маску индикатора вертикальных линий сетки для меток
// горизонтальной шкалы
QRegion
QDragZoomSvc::addHorTicks(QRegion rw, QwtScaleDiv::TickType tt)
{
    // получаем указатель на график
    QwtPlot* plt = zoom->plot();
    // получаем список основных меток горизонтальной шкалы
...
// Добавление в маску индикатора горизонтальных линий сетки для меток
// вертикальной шкалы
QRegion
QDragZoomSvc::addVerTicks(QRegion rw, QwtScaleDiv::TickType tt)
{
    // получаем указатель на график
    QwtPlot* plt = zoom->plot();
...
void
QDragZoomSvc::showDragWidget(QPoint evpos)
{
    // получаем указатель на график
    QwtPlot* plt = zoom->plot();
    // получаем геометрию графика
    ...
    QRect wg(
        pg.x() + 1 + evpos.x() - scp_x, pg.y() + 1 + evpos.y() - scp_y, ww, wh);
    ...
    {
        // добавляем в маску вертикальные линии сетки для делений горизонтальной
        // шкалы
...
void
QDragZoomSvc::applyDrag(QPoint evpos)
{
    // получаем указатель на график
    QwtPlot* plt = zoom->plot();
    // получаем геометрию канвы графика
    ...
    //     (чтобы график относительно границ переместился вправо, сами границы
    //     следует сместить влево)
...
void
QDragZoomSvc::dragMouseEvent(QEvent* event)
{
    // создаем указатель на событие от мыши
    QMouseEvent* mEvent = static_cast<QMouseEvent*>(event);
    // в зависимости от типа события вызываем соответствующий обработчик
...
void
QDragZoomSvc::startDrag(QMouseEvent* mEvent)
{
    // фиксируем исходные границы графика (если этого еще не было сделано)
    zoom->fixBounds();
    // если в данный момент еще не включен ни один из режимов
    if (zoom->regim() == QwtChartZoom::ctNone)
    {
        ...
        if (scp_x >= 0 && scp_x < cg.width() && scp_y >= 0
            ...
            if (mEvent->button() == Qt::RightButton)
            {
                ...
                // определяем текущий масштабирующий множитель по горизонтальной
                // оси (т.е. узнаем на сколько изменяется координата по шкале x
                ...
                // получаем основную вертикальную шкалу
                QwtPlot::Axis mY = zoom->masterV();
                // определяем текущий масштабирующий множитель по вертикальной
                // оси (аналогично)
...
void
QDragZoomSvc::procDrag(QMouseEvent* mEvent)
{
...
void
QDragZoomSvc::endDrag(QMouseEvent* mEvent)
{
    // если включен режим изменения масштаба или режим перемещения графика
    if (zoom->regim() == QwtChartZoom::ctDrag)
